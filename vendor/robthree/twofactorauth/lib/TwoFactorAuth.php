...
class TwoFactorAuth
{
    ...
    {
        if ($qrcodeprovider !== null && !($qrcodeprovider instanceof IQRCodeProvider)) {
            throw new TwoFactorAuthException('QRCode provider must implement IQRCodeProvider');
        }

        if ($rngprovider !== null && !($rngprovider instanceof IRNGProvider)) {
            throw new TwoFactorAuthException('RNG provider must implement IRNGProvider');
        }

        if ($timeprovider !== null && !($timeprovider instanceof ITimeProvider)) {
            throw new TwoFactorAuthException('Time provider must implement ITimeProvider');
        }
     ...
     * @return void
     */
    public function ensureCorrectTime($timeproviders = null, $leniency = 5)
    {
        ...
        } elseif (!is_array($timeproviders)) {
            throw new TwoFactorAuthException('Timeproviders must be an array or null');
        }
